...
local defaults = {
	...
	AllowIdle = false, -- true if models are permitted to use their natural animation which include idle behavior
}
...
function adapt:InitOptions()
	...
	opt:SetScript("OnShow",function() opt.refresh() C_Timer.After(0,adapt.RefreshAll) opt:SetScript("OnShow",nil) end)
end
...
function adapt:OptionsCheckOnClick()
	if self.variable then
		...
		adapt:ShapeAllModels()
		adapt:ColorAllBackLayers()
		if self.variable=="HideAnon" or self.variable=="AllowIdle" then
			adapt:RefreshAll()
		end
