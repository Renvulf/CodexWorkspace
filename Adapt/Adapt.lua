...
adapt.isModernClient = select(4,GetBuildInfo())>=120000
...
function adapt.NewSetPortraitTexture(texture,unit,disableMasking)
	if UnitExists(unit) and not texture.noAdapt and not adapt.Blackcache[texture] and not adapt.forbid[texture:GetName()] then
		...
		-- if guid is a secret value, always update (this isn't super frequent) without checking or saving guid
		if adapt.isModernClient and issecretvalue(guid) then
...
function adapt.CreateModel(texture,unit)
	...
	portrait.backLayer = texture:GetParent():CreateTexture(nil,drawLayer,nil,-1) --drawLayer=="OVERLAY" and "ARTWORK" or drawLayer)
	...
	portrait.modelLayer:SetModelDrawLayer(drawLayer) -- put model at drawLayer of texture

	portrait.maskLayer = texture:GetParent():CreateTexture(nil,drawLayer,nil,1)
...
--		print("adapt SetUnit",unit,GetTime())
		...
		-- Vulpera stand animation is kinda bugged; AllowIdle option allows models to do their natural behavior which includes "idle" behavior (worgen looking off camera etc)
		if not AdaptSettings.AllowIdle then
...
SlashCmdList["ADAPT"] = function(msg)
	if msg=="debug" then
		...
		for texture,portrait in pairs(adapt.portraits) do
			...
			print(texture:GetName() or "nil child of "..(texture:GetParent():GetName() or "nil"),"\""..model.."\"",texture:GetDrawLayer(),(portrait.modelLayer:GetModelDrawLayer()))
		end
	else
